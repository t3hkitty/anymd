...
## 🌌 n8n Cloud Folder integration & Mobile Localhost wrapper

Anymd now supports loading and syncing vaults from cloud-based systems using local n8n routing rules or sandboxed environments:

### 1. n8n Cloud Folder wrapper
Select **n8n Local Webhook Sync (Cloud Folders)** as your vault source to bypass browser filesystem directory picker limits.
- **Outbound triggers**: When loading or refreshing, Anymd posts `LIST_FILES` or `GET_FILE` payloads to `http://localhost:5678/webhook/anymd-action`.
- **Cloud pipelines**: n8n intercepts these actions, routes the files through your synced Google Drive, Nextcloud, or WebDAV accounts, and hydrates Anymd's active UI state.
