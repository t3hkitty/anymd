...
import { CLOUD_PROVIDER_PRESETS, buildRelLinkRootForAccount, saveCloudAccounts } from '../plugins/cloudAccountManager';
import { testRealWebDAVConnection, type WebDAVTestResult } from '../plugins/webdavSyncEngine';
import { X, Cloud, Plus, Trash2, ShieldCheck, Sparkles, Key, Lock, Unlock, HardDrive, Pencil, AlertCircle, FolderSync, Info, ExternalLink } from 'lucide-react';
...
export const WebDAVAccountManagerModal: React.FC<WebDAVAccountManagerModalProps> = ({
  ...
  const [username, setUsername] = useState('lorikitty@gmail.com');
  ...
  const handleOpenAddForm = () => {
    ...
    setUsername('lorikitty@gmail.com');
    setTokenOrPassword('');
    setApiKey('');
        ...
        <div className="p-6 space-y-5 flex-1 overflow-y-auto">
          ...
          {!isEditing ? (
            ...
            <form onSubmit={handleSaveAccount} className="space-y-4">
                ...
                <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/50 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-300 font-mono flex items-center space-x-1.5">
                      ...
                      <span>GOOGLE DRIVE API & RCLONE WEBDAV INTEGRATION</span>
                    </span>
                    <a
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed">
                    You can connect Google Drive via <strong>Option A (API OAuth Token / Client ID)</strong> or <strong>Option B (Local rclone WebDAV bridge)</strong>:
                  </p>
                  ...
                  <div className="space-y-2 text-xs font-mono">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="ya29.a0Ax... (Paste OAuth Token or Client Secret)"
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-emerald-500/50 text-xs text-amber-300 focus:outline-none"
                      />
                    </div>

                    <p className="text-[10px] text-slate-400 leading-snug pt-1">
                      💡 <strong>How to get Google Drive API Keys:</strong> Go to Google Cloud Console &rarr; Enable Google Drive API &rarr; Create Credentials (OAuth 2.0 Client ID) &rarr; Copy Access Token above. Or run <code className="text-amber-300">rclone serve webdav gdrive: --addr :8080</code>.
                    </p>
                  </div>
                </div>
              )}
              ...
              {/* TorBox Specific API Key Input Field */}
              {presetId === 'torbox' && (
                <div className="p-3.5 rounded-2xl bg-indigo-950/40 border border-indigo-500/40 space-y-1">
                  <label className="block text-xs font-bold text-indigo-300 uppercase tracking-wider flex items-center space-x-1">
                    <Key className="w-3.5 h-3.5 text-amber-400" />
                    <span>TorBox Secret API Key Authentication</span>
                  </label>
                  <input
                    type="password"
                    value={apiKey}
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-indigo-500/50 text-xs text-amber-300 font-mono focus:outline-none"
                    required
                  />
                  <p className="text-[10px] text-slate-400">TorBox API key authenticates WebDAV requests to webdav.torbox.app</p>
                </div>
              )}
